...
const photoBooth = (function () {
    ...
    public.processPic = async function (photoStyle, result) {
        ...
        $.ajax({
            ...
            success: (data) => {
        	    ...
        	    if (photoStyle === 'collage') {
                        console.log('COLLAGE LED');
                        var request = new XMLHttpRequest();
                        request.open("GET","http://10.0.0.2/COLLAGE");
                        request.send();
                    }else{
                        console.log('PIC LED');
                        var request = new XMLHttpRequest();
                        request.open("GET","http://10.0.0.2/PIC");
                        request.send();
                     };
    ...
    public.startCountdown = function (start, element, cb) {
       ...
       if (start == 5) {
               console.log('five');
               var request = new XMLHttpRequest();
               request.open("GET","http://10.0.0.2/BACK");
               request.send();
       }
       if (start == 2) {
               console.log('two');
               var request = new XMLHttpRequest();
               request.open("GET","http://10.0.0.2/TWO");
               request.send();
       }
